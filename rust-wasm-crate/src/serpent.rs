...
impl Serpent256Ctr {
    ...
    pub fn apply_ctr(&self, base_nonce: &[u8; 16], chunk_index: u64, data: &mut [u8]) {
        ...
        while offset < data.len() {
            ...
            if to_xor == BLOCK_SIZE {
                // 128-bit SIMD vector keystream XOR
                let data_vec = u128::from_ne_bytes(data[offset..offset + 16].try_into().unwrap());
                let key_vec = u128::from_ne_bytes(keystream_block.as_slice().try_into().unwrap());
                data[offset..offset + 16].copy_from_slice(&(data_vec ^ key_vec).to_ne_bytes());
            } else {
                for i in 0..to_xor {
