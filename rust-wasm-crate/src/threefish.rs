...
impl Threefish1024 {
    ...
    pub fn new(key32: &[u8; 32], tweak16: &[u8; 16]) -> Self {
        ...
        Self::init_with_k_and_tweak(k, tweak16)
    }

    /// Initialize Threefish-1024 directly with a strict 128-byte (1024-bit) key
    pub fn from_1024_bit_key(key128: &[u8; 128], tweak16: &[u8; 16]) -> Self {
        let mut k = [0u64; 17];
        for i in 0..16 {
            let offset = i * 8;
            k[i] = u64::from_le_bytes(key128[offset..offset + 8].try_into().unwrap());
        }
    ...
    fn init_with_k_and_tweak(k: [u64; 17], tweak16: &[u8; 16]) -> Self {
    ...
    pub fn apply_ctr(&self, base_nonce: &[u8; 16], chunk_index: u64, data: &mut [u8]) {
        ...
        while offset < data.len() {
            ...
            let mut i = 0;
            // 128-bit SIMD vector chunk XORing
            while i + 16 <= to_xor {
                let data_vec = u128::from_ne_bytes(data[offset + i..offset + i + 16].try_into().unwrap());
                let key_vec = u128::from_ne_bytes(keystream_block[i..i + 16].try_into().unwrap());
                data[offset + i..offset + i + 16].copy_from_slice(&(data_vec ^ key_vec).to_ne_bytes());
                i += 16;
            }
            while i < to_xor {
