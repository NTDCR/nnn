  ...
  gpuDevicePromise = (async () => {
    try {
      ...
      device.lost.then((info) => {
        console.warn('WebGPU device lost, invalidating cached device:', info?.message);
...
export class WebGpuCascadeEngine implements WebGpuEngineInstance {
  ...
  public async encryptChunkChaCha(
    ...
    if (startCounter < 0 || !Number.isSafeInteger(startCounter)) {
      throw new Error('Invalid startCounter');
    }
    ...
    let uniformArray: ArrayBuffer | null = null;
    ...
    try {
      ...
      uniformArray = new ArrayBuffer(64);
      ...
      if (uniformArray) {
        new Uint8Array(uniformArray).fill(0);
      }
