...
export async function runSelfVerificationTests(
  ...
  // 18. SIMD Acceleration: 32-Lane Bit-Slice & 64-bit Vector Keystream Invariance
  try {
    const t0 = performance.now();
    // 1. Verify 32-lane bit-slice SIMD against canonical S-box definitions across all 8 S-boxes
    const { applySboxBitsliceSIMD, SBOX } = await import('./serpent256.ts');
    let sboxParityPassed = true;
    const testR0 = 0x12345678, testR1 = 0x9abcdef0, testR2 = 0x0fedcba9, testR3 = 0x87654321;
    const simdOut: [number, number, number, number] = [0, 0, 0, 0];

    for (let sIdx = 0; sIdx < 8; sIdx++) {
      applySboxBitsliceSIMD(sIdx, testR0, testR1, testR2, testR3, simdOut);
      // Canonical bit-by-bit check
      const refS = SBOX[sIdx];
      let o0 = 0, o1 = 0, o2 = 0, o3 = 0;
      for (let bit = 0; bit < 32; bit++) {
        const inVal = ((testR0 >>> bit) & 1) |
                      (((testR1 >>> bit) & 1) << 1) |
                      (((testR2 >>> bit) & 1) << 2) |
                      (((testR3 >>> bit) & 1) << 3);
        const outVal = refS[inVal];
        o0 |= (outVal & 1) << bit;
        o1 |= ((outVal >>> 1) & 1) << bit;
        o2 |= ((outVal >>> 2) & 1) << bit;
        o3 |= ((outVal >>> 3) & 1) << bit;
      }
      if (simdOut[0] !== (o0 >>> 0) || simdOut[1] !== (o1 >>> 0) || simdOut[2] !== (o2 >>> 0) || simdOut[3] !== (o3 >>> 0)) {
        sboxParityPassed = false;
        break;
      }
    }

    // 2. Verify SIMD vector keystream roundtrip on 256-byte payload (across multiple 128B blocks)
    const { Threefish1024 } = await import('./threefish1024.ts');
    const tfKey = hexToBytes('55'.repeat(128), 128);
    const tfTweak = new Uint8Array(16).fill(0x33);
    const tfCipher = new Threefish1024(tfKey, tfTweak);
    const tfData = new Uint8Array(256);
    for (let i = 0; i < 256; i++) tfData[i] = (i * 13) & 0xff;
    const tfWork = new Uint8Array(tfData);
    const tfNonce = new Uint8Array(16).fill(0x44);
    tfCipher.processCtr(tfWork, tfNonce, 0);
    const tfDec = new Uint8Array(tfWork);
    tfCipher.processCtr(tfDec, tfNonce, 0);
    const tfPassed = bytesToHex(tfDec) === bytesToHex(tfData);

    const t1 = performance.now();
    const passed = sboxParityPassed && tfPassed;

    report({
      suite: 'SIMD Acceleration Architecture',
      name: '32-Lane Bit-Slice & 64-bit Vector Keystream Invariance',
      passed,
      expectedHex: 'Bit-exact bit-slice SIMD truth tables & vector keystream invariance',
      actualHex: passed
        ? 'Bit-exact bit-slice SIMD truth tables & vector keystream invariance'
        : `SBOX parity=${sboxParityPassed}, Threefish vector=${tfPassed}`,
      executionTimeMs: Number((t1 - t0).toFixed(2)),
    });
  } catch (err) {
    report({
